import { useEffect, useState } from 'react';

function useToggle(initValue: boolean) {
  const [isOn, toggle] = useState(initValue);

  const setIsOn = (initValue: boolean) => {
    toggle(initValue);
  };

  const reset = () => {
    toggle(false);
  };

  return { isOn, toggle, setIsOn, reset };
}

export const VisibilityToggle = () => {
  const { isOn, setIsOn } = useToggle(false);
  return (
    <div style={{ marginTop: '50px' }}>
      <div style={{ display: 'flex' }}>
        <button onClick={() => setIsOn(true)}>Показать</button>
        <button onClick={() => setIsOn(false)}>Скрыть</button>
      </div>
      <h2>{isOn ? '🎉 Это секретное сообщение!' : ''}</h2>
    </div>
  );
};

export const LightSwitch = () => {
  const { isOn, toggle } = useToggle(false);
  return (
    <div>
      <h2>{isOn ? '💡 Свет включен' : '🌙 Свет выключен'}</h2>
      <button onClick={() => toggle(!isOn)}>Переключить свет</button>
    </div>
  );
};

export const NotificationSwitch = () => {
  const { isOn, toggle, setIsOn, reset } = useToggle(true);

  return (
    <div style={{ marginTop: '50px' }}>
      <h2>Настройки уведомлений</h2>
      <h2>{isOn ? '🔔 Уведомления включены' : '🔕 Уведомления выключены'}</h2>
      <div style={{ display: 'flex' }}>
        <button onClick={() => toggle(!isOn)}>Переключить</button>
        <button onClick={() => setIsOn(true)}>Включить</button>
        <button onClick={() => reset()}>Сбросить по умолчанию</button>
      </div>
    </div>
  );
};

export const TogglePage = () => {
  return (
    <div>
      <LightSwitch />
      <VisibilityToggle />
      <NotificationSwitch />
      <TitleEditor />
      <GreetingCard />
    </div>
  );
};

function useText(initValue: string) {
  const [text, setText] = useState(initValue);

  const clear = () => {
    setText('');
  };

  const toUpperCase = () => {
    setText(text.toUpperCase());
  };

  const toLowerCase = () => {
    setText(text.toLowerCase());
  };

  return { text, setText, clear, toUpperCase, toLowerCase };
}

export const TitleEditor = () => {
  const { text, setText, clear, toUpperCase, toLowerCase } =
    useText('Заголовок статьи');

  return (
    <div style={{ marginTop: '50px' }}>
      <h2>{text || 'Пусто'}</h2>
      <div style={{ display: 'flex' }}>
        <button onClick={() => toUpperCase()}>ВЕРХНИЙ РЕГИСТР</button>
        <button onClick={() => toLowerCase()}>нижний регистр</button>
        <button onClick={() => setText('Новый заголовок')}>Изменить</button>
        <button onClick={() => clear()}>Очистить</button>
      </div>
    </div>
  );
};

export const GreetingCard = () => {
  const { text, setText, clear, toUpperCase, toLowerCase } = useText('Привет!');

  return (
    <div>
      <h2>💬 {text || '...'}</h2>
      <button onClick={() => toUpperCase()}>ГРОМКО</button>
      <button onClick={() => toLowerCase()}>тихо</button>
      <button onClick={() => setText('Добро пожаловать')}>
        Сказать 'Добро пожаловать!'
      </button>
      <button onClick={() => clear()}>Молчать</button>
    </div>
  );
};
